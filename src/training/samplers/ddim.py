...
class DDIMSampler(nn.Module):
    ...
    def sample(
        ...
        for step, (g_cur, g_next) in enumerate(zip(gammas[:-1], gammas[1:])):
            ...
            noise_pred = (x_cur - g_cur * x0) / torch.sqrt(1 - g_cur**2)

            # Compute x_next deterministically using DDIM update rule
            x_next = g_next * x0 + torch.sqrt(1 - g_next**2) * noise_pred
